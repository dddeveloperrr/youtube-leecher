...
public class youtube_source {
	...
	public String extract_scrip_section_from_youtube() throws Exception {
		...
		m2.find();
		File_Title = m2.group(0);
		File_Title = File_Title.substring(9).replace("\"","");
		File_Title = java.net.URLEncoder.encode(File_Title, "UTF-8").replace("+", "%20");
		sb.append(m.group(0));
		return sb.toString();
	}
	 ...
	 */
	public ArrayList<String> extract_urls() throws Exception {
		...
		while (m2.find()) {
			...
			urlsss.add(m2.group().replace("\\u0026", "")+ "&title=" + URLEncoder.encode(File_Title, "UTF-8"));
		}
		return urlsss;
	}
	...
	/*
	 با استفاده از این تابع، میایم و حجم تک تک ویدیو هارو با استفاده از لینک
	 های استخراج شده در میاریم
	 */
	public int Get_File_Size (String File_URL) throws Exception {
		
		URL url = new URL(File_URL);
		int Content_size = ((HttpURLConnection) url.openConnection()).getContentLength();
		return Content_size; 
	}
}
